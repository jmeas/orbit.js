...
var Cache = Class.extend({
  init: function(schema, options) {
    ...
    var _this = this;
    this._relatedInverseLinksProcessor = new RelatedInverseLinksProcessor(schema, function(path) { return _this.retrieve(path); });
  ...
  transform: function(operation) {
    ...
    var performDependentOps = function() {
      dependentOperations.forEach(function(operation) {
        _this.transform(operation, false);
      });
      dependentOperations = [];
    };
    var inverse;
    ...
    if (op === 'remove' || op === 'replace') {
      if (this.maintainInverseLinks) {
        pushOps(this._relatedInverseLinksProcessor.before(normalizedOperation));
      }
    ...
    if (op === 'add' || op === 'replace') {
      ...
      if (this.maintainInverseLinks) {
        if (op === 'replace') {
          pushOps(this._relatedInverseLinksProcessor.after(normalizedOperation.spawn({
            op: 'add',
            path: path,
            value: value
          })));
          ...
          pushOps(this._relatedInverseLinksProcessor.after(normalizedOperation));
        }
      }
    }
